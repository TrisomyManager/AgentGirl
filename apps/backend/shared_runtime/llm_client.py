"""Unified async LLM client for all companion-ai modules (P1-E 物理搬迁完成 / V2.1).

Originally lived in ``shared/llm_client.py``; that path is now a deprecated
shim re-exporting from this module. Business modules should consume LLM
capability via ``LLMClientProtocol`` / dependency injection (ADR-006), and
only host glue (``main.py`` / orchestrator wiring) should import this
...
logger = structlog.get_logger("shared_runtime.llm_client")

# Persist next to apps/backend/.env so cwd does not change where config is read/written.
...
class LLMClient:
    ...
    async def _generate_openai(
        ...
        if not choices:
            raise RuntimeError(
                f"LLM returned empty choices. Response keys: {list(data.keys())}. "
                f"Model: {model}. Raw: {str(data)[:300]}"
            )
        choice = choices[0]
        return {
    ...
    async def _generate_openai_stream(
        ...
        async with self.http.stream("POST", url, json=payload, headers=headers) as resp:
            ...
            async for line in resp.aiter_lines():
                ...
                if not choices:
                    logger.debug(
                        "llm.openai_stream_empty_choices",
                        model=model,
                        keys=list(obj.keys()),
                    )
                    continue
