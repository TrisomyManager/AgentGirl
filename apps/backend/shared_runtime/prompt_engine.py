...
P1-F (V2.2): 物理实现从 ``shared.prompt_engine`` 搬迁至此.
``shared.prompt_engine`` 现为 deprecated re-export shim.
"""
...
_DEFAULT_NAME = "小暖"
_DEFAULT_BASE_PROMPT = "你是一个温柔体贴、善于倾听的小暖。请用自然温暖的中文回复。"
...
def build_base_system_prompt(persona: Optional[PersonaProfile] = None) -> str:
    ...
    return f"你是{persona.name}，一个温柔体贴、善于倾听的小暖。请用自然温暖的中文回复。"
...
def build_conversation_system_prompt(
    ...
    parts: List[str] = [f"你是{name}，一个温柔体贴的小暖。请始终用自然流畅的中文回复，语气亲切温暖。"]
    ...
    if memory and memory.working_memory:
        ...
        if wm.session_digest:
            wm_bits.append(f"本段对话摘要：{wm.session_digest}")
    ...
    parts.append(
        "【可用能力】你可以帮用户完成以下操作：查天气、设置提醒、倒计时、搜索信息、查询设备状态。"
        "当用户要求这些操作时，直接确认并执行，不需要说「让我来帮你」。"
    )
