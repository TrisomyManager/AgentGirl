...
| `device_coordination` | ✅ Lite Mode | ✅ | 设备注册与低危命令网关；Lite 下内存注册表 + 命令存储，MQTT 可选；完整模式用 PostgreSQL + MQTT |
| `gateway_adapter` | ❌ (直连核心) | ✅ | 多平台网关；本地 App 直连 core_orchestrator |
| `frontend_sdk` | ✅ 原生 | — | 独立 App SDK |

**本地最小启动集合：** 单体 `uvicorn main:app --port 8000`（Lite Mode）已挂载 `device_coordination` 路由；也可用多进程拆分：
...
验证 PC 端可跑 `apps/pc-client/sim_client.py`（`pip install httpx` 或 `pip install -e apps/pc-client`）对同一后端轮询并执行白名单命令。
