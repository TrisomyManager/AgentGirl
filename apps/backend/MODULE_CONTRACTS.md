...
```
apps/web (Vue 3) 鈹€鈹€HTTP/WebSocket鈹€鈹€鈻?main.py:8000 (缁熶竴鍏ュ彛)
                                              鈹?
...
| `/device/send_command` | POST | `{user_id, device_id?, command, payload?}` | command 对象含 `command_id`、状态等 | 白名单：`ping`、`show_notification`、`open_url`（仅 http/https）、`get_device_status`；未指定 `device_id` 时若该用户仅一台在线设备则自动选中 |
| `/device/command_result` | POST | `{command_id, status, result?, error?}` | 更新后的 command | 客户端上报 `succeeded` / `failed` / `denied` / `timeout` |
| `/device/commands` | GET | `?user_id=&device_id=` | 命令历史列表 | Lite Mode 下为进程内存储 |
| `/device/commands/{command_id}` | GET | - | 单条命令 | 按 ID 查询 |
| `/device/broadcast` | POST | `{user_id, command, payload}` | `{success, sent_to[]}` | 骞挎挱鎸囦护 |

**Lite Mode（`COMPANION_LITE_MODE=true`）**：设备注册表与待下发命令存于进程内内存；MQTT 不可用时代码仍将命令标为已投递，供客户端轮询 `GET /device/commands`（按 `user_id` / 状态过滤以实际 API 为准）。

**渚濊禆鐨勫閮ㄦ湇鍔?*锛?
- 完整部署：MQTT broker + PostgreSQL（设备表）
- Lite：无外部依赖

**Orchestrator / 工具**：`Intent.DEVICE_COMMAND` 经 `action_executor` 工具 `list_devices`、`device_ping`、`device_notify`、`device_open_url` 调用上述 HTTP，不直连拼凑 URL。
...
curl -X POST http://localhost:8105/device/register \
  ...
  -d '{"device_id":"pc-001","user_id":"user_001","device_type":"pc","device_name":"我的电脑","platform":"app","capabilities":["ping"]}'

# 鍙戦€佹寚浠?
curl -X POST http://localhost:8105/device/send_command \
  -H "Content-Type: application/json" \
  -d '{"user_id": "default", "command": "ping", "payload": {}}'
```
...
## 11. avatar_runtime / digital human presentation contract

`avatar_runtime` is a proposed presentation module, not an active core brain module. Its contract exists so future digital-human work stays inside a clear boundary.

### Responsibility

| Area | Owner |
|------|-------|
| Conversation turn, intent routing, tool decisions | `core_orchestrator` |
| Persona, tone, emotion state | `persona_engine` |
| Long-term memory and user facts | `memory_system` |
| ASR, TTS, realtime voice provider integration | `voice_layer` |
| Tools, reminders, proactive push, device tools | `action_executor` |
| Device registration and command delivery | `device_coordination` |
| Avatar state, lip-sync presentation, subtitles, render backend adapters | `avatar_runtime` |

### Proposed endpoints

These endpoints are future contracts. Do not implement them until the existing chat, memory, voice, tool, and device-operation loops are stable.

| Endpoint | Method | Request | Response | Purpose |
|----------|--------|---------|----------|---------|
| `/avatar/session` | POST | `{user_id, session_id, backend?}` | `{avatar_session_id, backend, state}` | Create avatar render session |
| `/avatar/session/{id}/event` | POST | `{event_type, payload, timestamp}` | `{accepted, state}` | Feed presentation event |
| `/avatar/session/{id}/interrupt` | POST | `{reason}` | `{interrupted, state}` | Stop current presentation |
| `/avatar/backends` | GET | - | `{backends[]}` | List available render backends |
| `/avatar/session/{id}/state` | GET | - | `{state, backend, last_event}` | Inspect current render state |
| `/avatar/session/{id}/events` | WebSocket | JSON events | JSON events | Realtime render/subtitle/status stream |
...
`avatar_runtime` may import:

- `shared_contracts`
- `shared_runtime`

`avatar_runtime` must not import internal implementation from:
...
Cross-module communication must use events or HTTP/WebSocket contracts.

See [`docs/avatar-runtime-architecture.md`](docs/avatar-runtime-architecture.md) for the full module boundary and adoption plan.
---
