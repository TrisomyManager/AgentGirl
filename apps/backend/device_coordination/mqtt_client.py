...
class DeviceMQTTClient:
    ...
    async def publish_command_envelope(self, device_id: str, command: Dict[str, Any]) -> None:
        """Publish the full gateway command record for rich clients (risk, expiry, etc.)."""
        if self._client is None:
            raise RuntimeError("MQTT client not connected")
        message = {
            "event_id": str(uuid.uuid4()),
            "source_module": "device_coordination",
            "event_type": "device:command_envelope",
            "device_id": device_id,
            "command": command,
        }
        topic = TOPIC_COMMAND.format(device_id=device_id)
        await self._client.publish(topic, payload=json.dumps(message, ensure_ascii=False))
        logger.info(
            "mqtt_client.command_envelope_published",
            device_id=device_id,
