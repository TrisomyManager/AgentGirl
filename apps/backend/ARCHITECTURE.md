...
> **鏍稿績鍘熷垯**锛氬绾﹀厛琛屻€佸涓绘棤鍏炽€乣core_orchestrator` 涓?`apps/web` 浠呮槸鍙傝€冮泦鎴?demo锛屼笉鏄繀閫夈€?
...
| `apps/web` | 鍙傝€?UI / 楠屾敹 demo | Vue 3 璋冭瘯鍙?+ Live2D 娓叉煋绀轰緥 | Web | 鈥?|
...
| device_coordination | `GET /device/list/{user_id}` | 列出用户设备（Lite / 全量均可用） |
| device_coordination | `POST /device/send_command` 等 | Device Operation Gateway MVP：白名单命令、审计、客户端 `command_result` 回传 |
...
The old Hermes and AIRI source trees are no longer part of this repository. They were useful as early references, but the active architecture is now owned by `apps/backend/` (and sibling `apps/web`, `apps/pc-client`). Do not import from, test, or ask Agents to inspect those upstream projects during normal work.
## 7. 鍗曡鑹叉繁搴﹀吇鎴愯璁¤鐐?
...
## 9. Avatar Runtime / Digital Human Pipeline

`avatar_runtime` is the proposed presentation layer for Xiaonuan's visible body. It is where Live2D, 2D video avatar, 3D avatar, and future WebRTC digital-human sessions should converge.

The module boundary is strict:

- `core_orchestrator` owns the conversation turn, intent, memory coordination, and tool decisions.
- `persona_engine` owns Xiaonuan's personality and emotional state.
- `memory_system` owns long-term memory and user facts.
- `voice_layer` owns ASR/TTS and realtime voice providers.
- `action_executor` owns tools, reminders, proactive push, and device-facing actions.
- `avatar_runtime` only owns visual/render presentation, subtitle timing, lip-sync state, avatar backend selection, and interruption presentation.

Digital-human projects such as OpenTalking are useful references for pipeline separation, backend adapters, WebRTC transport, subtitle/status events, and interruption handling. They are not upstream dependencies of this repository. Do not copy their full source tree into this workspace.

The detailed architecture is maintained in [`docs/avatar-runtime-architecture.md`](docs/avatar-runtime-architecture.md).
