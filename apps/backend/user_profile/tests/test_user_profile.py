...
async def test_invalid_role_id_fallback_to_default_persona_no_warn(caplog: pytest.LogCaptureFixture) -> None:
    """给定 invalid role_id, _recall_memory_monolithic 必须 fallback 到默认 YAML 人格；
    persona_engine 不产生 warning（预期缺失 role 仅 info，真正损坏仍 warning）。"""
    ...
    # Invalid role_id falls back to default persona YAML (``default.yaml`` → 小暖).
    assert persona.name == "小暖", f"Expected default persona name, got {persona.name!r}"
