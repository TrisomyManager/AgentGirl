...
def test_find_by_keyword_device_phrases() -> None:
    registry = get_registry()
    assert registry.find_by_keyword("我的电脑在线吗") == "list_devices"
    assert registry.find_by_keyword("我的设备有哪些") == "list_devices"
    assert registry.find_by_keyword("Ping 一下我的电脑") == "device_ping"
    assert registry.find_by_keyword("给我的电脑发个通知：测试一下") == "device_notify"
    assert registry.find_by_keyword("让我的电脑打开 https://example.com") == "device_open_url"
...
@pytest.mark.asyncio
async def test_device_notify_extracts_text_after_colon(monkeypatch: pytest.MonkeyPatch) -> None:
    from action_executor.handlers import device_notify_handler

    async def _fake(path: str, method: str = "GET", json_data: dict | None = None):
        assert json_data and json_data.get("payload", {}).get("text") == "测试一下"
        return {
            "success": True,
            "_http_status": 200,
            "command": {"device_id": "pc-1", "command_id": "c1"},
        }

    monkeypatch.setattr("action_executor.handlers._call_device_api", _fake)
    r = await device_notify_handler({"user_id": "u1", "raw_text": "给我的电脑发个通知：测试一下"})
    assert r.ok
    assert "测试一下" in r.message


@pytest.mark.asyncio
async def test_device_open_url_extracts_from_raw_text(monkeypatch: pytest.MonkeyPatch) -> None:
    from action_executor.handlers import device_open_url_handler

    async def _fake(path: str, method: str = "GET", json_data: dict | None = None):
        assert json_data and "example.com" in json_data.get("payload", {}).get("url", "")
        return {"success": True, "_http_status": 200, "command": {"device_id": "pc-1"}}

    monkeypatch.setattr("action_executor.handlers._call_device_api", _fake)
    r = await device_open_url_handler({"user_id": "u1", "raw_text": "让我的电脑打开 https://example.com/path"})
    assert r.ok
    assert "example.com" in r.message


@pytest.mark.asyncio
async def test_list_devices_message_includes_heartbeat(monkeypatch: pytest.MonkeyPatch) -> None:
    from action_executor.handlers import list_devices_handler
            ...
            "devices": [
                {
                    ...
                    "device_id": "d1",
                    "is_online": True,
                    "last_heartbeat": "2026-03-15T10:30:00",
                    "capabilities": ["ping"],
                }
            ],
        }
    ...
    monkeypatch.setattr("action_executor.handlers._call_device_api", _fake)
    r = await list_devices_handler({"user_id": "u1"})
    assert r.ok
    assert "在线" in r.message
    assert "心跳" in r.message
...
async def test_query_memory_returns_formatted_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_recall(  # noqa: ANN001, ANN202
        *,
        session,
        user_id,
        query,
    ...
    async def _fake_list_memories(*, session, user_id, limit, offset=0, category=None):  # noqa: ANN001, ANN202
    ...
    # Mock working memory (snapshot must implement ``to_dict()`` like WorkingMemoryState).
    class _FakeWMState:
    ...
    _fake_wm = MagicMock()
    _fake_wm.snapshot = AsyncMock(return_value=_FakeWMState())
