...
def _companion_self_http_base() -> str:
    """Base URL for calling companion HTTP routes from the same deployment."""
    settings = get_settings()
    explicit = (settings.self_base_url or "").strip()
    if explicit:
        return explicit.rstrip("/")
    return f"http://127.0.0.1:{int(settings.service_port)}"


def _memory_use_in_process() -> bool:
    """Lite / monolithic single-process runs: avoid loopback HTTP so tests and offline dev stay stable."""
    s = get_settings()
    return bool(s.lite_mode or s.monolithic)
...
def _memory_entry_to_dict(entry: Any) -> Dict[str, Any]:
    ...
    if content is None:
        content = str(entry)
    imp = getattr(entry, "importance", 0.5)
    try:
        imp_f = float(imp) if imp is not None else 0.5
    except (TypeError, ValueError):
        imp_f = 0.5
    return {"content": content, "importance": imp_f}


async def _call_memory_recall_direct(
    user_id: str, query: str, session_id: Optional[str] = None
) -> List[Dict[str, Any]]:
    from memory_system.recall import recall_memory
    from shared_runtime.database import AsyncSessionLocal
    ...
    return [_memory_entry_to_dict(e) for e in result.entries]


async def _call_memory_list_direct(user_id: str, limit: int) -> List[Dict[str, Any]]:
    from memory_system.vector_store import list_user_memories
    from shared_runtime.database import AsyncSessionLocal

    async with AsyncSessionLocal() as session:
        entries = await list_user_memories(session=session, user_id=user_id, limit=limit)
    return [_memory_entry_to_dict(e) for e in entries]


async def _call_working_memory_direct(session_id: str) -> Optional[Dict[str, Any]]:
    from memory_system.working import get_working_memory

    state = await get_working_memory().snapshot(session_id)
    return state.to_dict()


async def _call_memory_recall(user_id: str, query: str, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
    """Semantic recall — in-process when lite/monolithic, else HTTP."""
    if _memory_use_in_process():
        try:
            return await _call_memory_recall_direct(user_id, query, session_id)
        except Exception as exc:
            logger.warning("query_memory.recall_direct_failed", user_id=user_id, error=str(exc))
            return []
    try:
        timeout = httpx.Timeout(8.0, connect=3.0)
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.post(
                f"{_companion_self_http_base()}/memory/recall",
                json={
                    "user_id": user_id,
                    "query": query,
                    "session_id": session_id,
...
async def _call_memory_list(user_id: str, limit: int = 10) -> List[Dict[str, Any]]:
    if _memory_use_in_process():
        try:
            return await _call_memory_list_direct(user_id, limit)
        except Exception as exc:
            logger.warning("query_memory.list_direct_failed", user_id=user_id, error=str(exc))
            return []
    try:
        timeout = httpx.Timeout(8.0, connect=3.0)
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.get(
                f"{_companion_self_http_base()}/memory/user/{user_id}/list",
                params={"limit": limit},
            )
            resp.raise_for_status()
            entries = resp.json()
...
async def _call_working_memory(session_id: str) -> Optional[Dict[str, Any]]:
    if _memory_use_in_process():
        try:
            return await _call_working_memory_direct(session_id)
        except Exception as exc:
            logger.warning("query_memory.working_direct_failed", session_id=session_id, error=str(exc))
            return None
    try:
        timeout = httpx.Timeout(5.0, connect=2.0)
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.get(f"{_companion_self_http_base()}/memory/working/{session_id}")
            resp.raise_for_status()
            return resp.json()
    except Exception as exc:
...
async def query_memory(params: Dict[str, Any]) -> ActionResult:
    ...
    Uses in-process memory_system calls when ``lite_mode`` or ``monolithic`` is
    enabled; otherwise calls memory HTTP APIs at ``self_base_url`` (env
    ``COMPANION_SELF_BASE_URL``) or ``http://127.0.0.1:{service_port}``.
...
# ---------------------------------------------------------------------------
# Device operation handlers — proxy to device_coordination via HTTP
# ---------------------------------------------------------------------------


async def _call_device_api(path: str, method: str = "GET", json_data: dict | None = None) -> dict | None:
    base = _companion_self_http_base()
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            try:
                ...
                logger.warning("device.proxy_invalid_json", path=path, status=resp.status_code)
                return None
            ...
            if not resp.is_success:
                logger.warning("device.proxy_failed", path=path, status=resp.status_code)
            return data
    except Exception as exc:
        logger.warning("device.proxy_error", path=path, error=str(exc))
        return None


def _format_device_heartbeat(iso_val: Any) -> str:
    if not iso_val:
        return ""
...
async def list_devices_handler(params: Dict[str, Any]) -> ActionResult:
    user_id = params.get("user_id") or "user_001"
    data = await _call_device_api(f"/device/list/{user_id}")
    if data is None:
        return ActionResult(ok=False, message="设备模块暂时不可用，稍后再试好吗？", data={"error": "device_module_unavailable"})
    if data.get("_http_status", 200) >= 400 or data.get("success") is False:
        return ActionResult(ok=False, message="设备模块暂时不可用，稍后再试好吗？", data={"error": "device_module_unavailable"})
    ...
    for d in devices:
        ...
        caps = ", ".join(d.get("capabilities", [])) or "无"
        hb = d.get("last_heartbeat")
        hb_s = f"，上次心跳 {_format_device_heartbeat(hb)}" if hb else ""
        lines.append(
            f"  • {d.get('device_name', d.get('device_id', '?'))} [{status}]{hb_s} — 能力：{caps}"
        )
    return ActionResult(ok=True, message=f"你绑定了 {len(devices)} 台设备：\n" + "\n".join(lines), data={"devices": devices})


def _interpret_device_send_command(resp: dict | None, message_for_ok: Callable[[dict], str]) -> ActionResult:
    """Normalize ``POST /device/send_command`` responses for action_executor tools."""
    if resp is None:
        return ActionResult(ok=False, message="抱歉，现在连不上你的设备。", data={"error": "device_unavailable"})
    ...
    if st >= 400 or resp.get("success") is False:
        return ActionResult(
            ok=False,
            message="指令未能下发，请确认设备在线后再试。",
            data={"error": "device_unavailable", "detail": resp.get("detail")},
        )
    cmd = resp.get("command") or {}
    return ActionResult(
        ok=True,
        message=message_for_ok(cmd),
        data={"command_id": cmd.get("command_id"), "status": cmd.get("status")},
    )


async def device_ping_handler(params: Dict[str, Any]) -> ActionResult:
    user_id = params.get("user_id") or "user_001"
    resp = await _call_device_api(
        "/device/send_command",
        method="POST",
        json_data={"user_id": user_id, "command": "ping", "payload": {}, "device_id": params.get("device_id")},
    )
    return _interpret_device_send_command(
        resp,
        lambda cmd: f"已向 {cmd.get('device_id', '设备')} 发送 Ping，等待响应中…",
    )


def _extract_notify_body_from_raw(raw: str) -> str:
    if not raw or not raw.strip():
        return ""
    for sep in ("通知：", "通知:", "提醒：", "提醒:", "——", " - "):
        if sep in raw:
...
async def device_notify_handler(params: Dict[str, Any]) -> ActionResult:
    user_id = params.get("user_id") or "user_001"
    ...
    return _interpret_device_send_command(
        resp,
        lambda cmd: f"已在设备 {cmd.get('device_id', '设备')} 上显示通知「{text}」",
    )


async def device_open_url_handler(params: Dict[str, Any]) -> ActionResult:
    import re

    user_id = params.get("user_id") or "user_001"
    url = (params.get("url") or "").strip()
    if not url:
        raw = params.get("raw_text") or ""
        m = re.search(r"https?://[^\s\u200b<>\"'）】]+", raw)
        if m:
            url = m.group(0).rstrip(").,;，。）」】\"'")
    if not re.match(r"^https?://", url):
        return ActionResult(ok=False, message="只允许打开 http 或 https 开头的安全链接哦。", data={"error": "invalid_url"})
    resp = await _call_device_api(
        "/device/send_command",
        method="POST",
        json_data={"user_id": user_id, "command": "open_url", "payload": {"url": url}, "device_id": params.get("device_id")},
    )
    return _interpret_device_send_command(
        resp,
        lambda cmd: f"已让设备 {cmd.get('device_id', '设备')} 打开 {url}",
    )
...
BUILTIN_ACTIONS = (
    ...
    {
        "name": "list_devices",
        "description": "List devices registered for the user (online / offline / capabilities).",
        "params_schema": {"user_id": "string"},
        "keywords": [
            "我的设备",
            "设备列表",
    ...
    },
    {
        ...
        "params_schema": {"user_id": "string", "device_id": "string (optional)"},
        "keywords": [
            "ping设备",
            "ping一下",
            "ping 一下",
    ...
    {
        "name": "device_notify",
        "description": "Send a local notification to a device.",
        "params_schema": {"user_id": "string", "text": "string", "device_id": "string (optional)"},
        "keywords": [
            "发通知",
            "设备通知",
            "电脑提醒",
    ...
    },
    {
        ...
        "description": "Open a URL on a device (http/https only).",
        "params_schema": {"user_id": "string", "url": "string", "device_id": "string (optional)"},
        "keywords": [
            "打开网址",
            "打开网页",
            "打开网站",
        ...
        "handler": device_open_url_handler,
        "needs_api_key": False,
        "category": "device",
        "requires_user_id": True,
    },
