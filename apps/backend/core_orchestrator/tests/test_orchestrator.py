...
from core_orchestrator.state_machine import (
    OrchestratorState,
    _try_action_executor,
    _try_device_actions_by_keyword,
    build_graph,
    build_initial_state,
    node_generate_response,
)
from shared_contracts.models import Platform, TurnContext, UserProfile
from action_executor.registry import ActionResult
...
@pytest.mark.asyncio
async def test_intent_router_heuristic_device_status_question() -> None:
    router = IntentRouter()
    router.settings.openai_api_key = None
    result = await router.classify("我的电脑在线吗")
    assert result.intent == Intent.DEVICE_COMMAND


@pytest.mark.asyncio
async def test_try_action_executor_accepts_device_command_intent() -> None:
    user = UserProfile(
        user_id="user-123",
        display_name="Alice",
        platform=Platform.APP,
    ...
    tc = TurnContext(
        ...
        session_id=str(uuid.uuid4()),
        user=user,
        user_message="我的电脑在线吗",
        platform=Platform.APP,
        has_voice=False,
    )
    with patch("action_executor.registry.get_registry") as mock_gr:
        reg = MagicMock()
        reg.find_by_keyword.return_value = "list_devices"
        reg.dispatch = AsyncMock(
            return_value=ActionResult(ok=True, message="你绑定了 1 台设备：测试", data={"devices": []})
        )
        mock_gr.return_value = reg
        import action_executor.handlers  # noqa: F401 — ensure builtins registered

        out = await _try_action_executor(tc, Intent.DEVICE_COMMAND.value)
        assert out is not None
        assert out["name"] == "list_devices"
        assert out["ok"] is True
        assert "设备" in out["message"]


@pytest.mark.asyncio
async def test_try_device_actions_by_keyword_for_chat_intent_phrase() -> None:
    user = UserProfile(
        user_id="user-123",
        display_name="Alice",
        platform=Platform.APP,
    ...
    with patch("action_executor.registry.get_registry") as mock_gr:
        ...
        reg.find_by_keyword.return_value = "list_devices"
        reg.dispatch = AsyncMock(
            return_value=ActionResult(ok=True, message="你绑定了 1 台设备", data={"devices": []})
        )
        mock_gr.return_value = reg
        import action_executor.handlers  # noqa: F401

        out = await _try_device_actions_by_keyword(tc)
        assert out is not None and out["name"] == "list_devices"


@pytest.mark.asyncio
async def test_node_generate_response_uses_device_tool_result_not_llm(
    sample_turn_context: TurnContext,
) -> None:
    """CHAT intent + device keyword still short-circuits to list_devices reply."""
    import action_executor.handlers  # noqa: F401

    sample_turn_context.user_message = "我的电脑在线吗"
    state: OrchestratorState = build_initial_state(sample_turn_context)
    state["intent"] = Intent.CHAT.value
    state["persona_profile"] = None
    state["memory_result"] = None
    state["emotion_state"] = None
    state["relationship_metrics"] = None

    with patch("action_executor.registry.get_registry") as mock_gr:
        reg = MagicMock()
        reg.find_by_keyword.return_value = "list_devices"
        reg.dispatch = AsyncMock(
            return_value=ActionResult(ok=True, message="电脑现在在线。", data={"devices": []})
        )
        mock_gr.return_value = reg

        out = await node_generate_response(state)
        assert out.get("assistant_message") == "电脑现在在线。"
        assert out.get("device_command_sent") is True
