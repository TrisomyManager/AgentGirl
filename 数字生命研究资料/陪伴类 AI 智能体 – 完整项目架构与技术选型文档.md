...
**面向陪伴类 AI / 数字生命系统的模块化技术参考方案**。
- 不绑定任何具体商业项目或 IP。
- 不与任何报价、排期、交付承诺挂钩。
- 文中的编排器、前端、渲染器和模块命名均为参考实现形态，可按实际宿主工程替换。

### 0.2 核心原则

| 原则 | 含义 | 落到代码 |
|------|------|---------|
| **契约先行（Contract-First）** | 模块之间用 Pydantic 模型 + 事件类型通信，不直接传内部对象 | 抽出 `shared_contracts/` 包，零依赖、可被任何宿主 import |
| **宿主无关（Host-Agnostic）** | 模块不假设跑在哪个编排器/前端/部署形态里 | 配置、LLMClient、日志由宿主注入；禁止模块反向 import `core_orchestrator` |
| **可拆 / 可装 / 可独立运行** | 每个模块都能单独被 `pip install`、单独跑 demo、单独写 README | 每个模块自带 `examples/`、`README.md`、最小依赖声明 |
| **去单角色硬编码** | 不绑死"小暖"或任何一个 IP | 人格/语音/画像走多角色加载器，角色 ID 由宿主传入 |
| **降级而非拒绝** | 模块缺依赖（无 LLM key、无 PG、无 Neo4j）时给出明确降级路径 | Lite Mode、内存兜底、空记忆继续对话 |

### 0.3 模块划分总览（按"可拆性"分类）

| 类别 | 模块 | 说明 |
|------|------|------|
| **通用能力模块（可拆）** | `persona_engine` / `memory_system` / `voice_layer` / `action_executor` / `gateway_adapter` / `device_coordination` | 可按独立包或独立服务设计，重点是契约稳定、依赖可替换 |
...
- 本文只描述推荐架构和选型依据，不表达任何模块是否已经完成。
- 文中模块名可作为实现参考，不要求具体工程逐字沿用。
- 成本、模型价格、第三方服务能力具有时效性，落地前需要重新核验。

---

## 1. 完整项目架构图（Mermaid）

> ⚠️ 下图是参考拓扑，用来表达陪伴类 AI 系统中各能力的关系。网关层、编排层、前端渲染器和模块边界都可按实际宿主工程调整。
```mermaid
flowchart TB
...
> **可拆性说明**：下表按能力边界组织，不代表具体项目进度。通用能力模块、辅助模块、契约/运行时层和参考集成层都可按宿主工程需要取舍或替换。
...
> LangGraph 适合作为参考编排器。实际宿主也可以使用 CrewAI、AutoGen、Temporal 或纯自研状态机，只要通过稳定契约消费各模块事件即可。
...
> **渲染路径建议**：实时陪伴场景优先考虑 **Live2D**（实时唇形 + 表情参数 + 动作触发，前端成本较低）；wan2.2 等视频生成方案更适合离线高保真短片。若宿主已有 Unity / Unreal / 自研 3D 渲染器，语音与动作模块只需输出口型、情感和动作意图事件，不绑定具体渲染器。
...
> 推荐所有业务模块都避免直连具体 LLM SDK，统一通过 `LLMClient` 类接口调用。宿主可替换为自家模型网关或本地模型，模块代码无需感知具体供应商。
...
| 模块 | 责任 | 设计要点 |
|------|------|------|
| `safety_guard` | 输入消毒、危机检测、越界拦截、降级响应 | 规则集可注入；可接入云端 moderation；输出应符合人格语气 |
| `user_profile` | 稳定用户画像：偏好、语言、角色选择、长期属性 | 与事件记忆解耦；支持导出、删除、脱敏和权限控制 |
| `onboarding` | 首会话/前 N 会话破冰流程 | 只产出结构化结果；具体 UI 与状态机由宿主决定 |

### 2.11 契约层与运行时层

| 模块 | 责任 | 依赖原则 |
|------|------|---------|
| `shared_contracts` | Pydantic 数据模型（Message / MemoryRecord / Persona / VoiceFrame / DeviceEvent…）+ 事件常量 | **零业务依赖**；任何模块都可 import；本身不 import 任何业务模块 |
| `shared_runtime` | `LLMClient` 接口、配置加载器、统一日志、Token 计费埋点 | 宿主可注入实现；提供默认实现（LiteLLM + dotenv + structlog）便于独立运行 |

> **反模式提醒**：
> - ❌ 业务模块直接 `from core_orchestrator.xxx import ...` —— 反向依赖，违反宿主无关原则。
> - ❌ 业务模块直接 `from openai import OpenAI` —— 绕过 `LLMClient` 抽象，无法被宿主替换。
> - ❌ 业务模块通过全局 prompt/config 隐式拿运行时配置 —— 建议改为显式注入。
...
> **模块化策略说明**：本表的"开源/集成/自研"评估仅作技术参考。具体工程可选择全量集成，也可只接入人格、记忆、语音、动作、安全、画像、onboarding 等任意子集。推荐始终保持契约先行、宿主无关、运行时可注入，避免被单一编排器、单一模型供应商或单一角色设定锁死。
...
| 维度 | 分析 |
|------|------|
| **决策** | 早期验证可采用 MQTT (Eclipse Mosquitto)，预留迁移到 NATS 的接口 |
...
| **缓解** | 早期单节点通常足够（目标 <1000 并发）；规模化时迁移 EMQX 集群或 NATS JetStream |
...
| **缓解** | 早期单实例通常足够；数据量增长后评估 AuraDB（托管）或 NebulaGraph |

### ADR-006: 模块独立可拆性 / 宿主无关原则

| 维度 | 分析 |
|------|------|
| **决策** | 业务模块建议满足「契约先行 + 宿主无关 + 可独立运行」三条约束 |
| **背景** | 陪伴类 AI 能力常被不同宿主复用。一旦出现"业务模块反向依赖编排器"或"硬绑定单一 LLM/单一角色"，模块会难以迁移到其他数字生命项目中 |
| **备选方案** | (A) 不做约束，先把功能跑通再说 — 等模块互相耦合后再拆，重构成本会指数级上升；(B) 用 monorepo 工具（pants/bazel）强制依赖图 — 当前规模过重；(C) 仅写文档约束 — 无强制力，靠 review 容易漂移 |
| **决策依据** | 采用「分层包结构 + import 静态检查 + 每模块自带 examples」三件套：① 建立 `shared_contracts`（零依赖）+ `shared_runtime`（可注入）两层；② 用 import-linter 或 ruff 规则禁止反向 import；③ 每个模块自带 `examples/run_standalone.py` 作为可拆性 CI 验证 |
| **三条硬约束** | 1) 业务模块禁止 `from core_orchestrator.* import *`；2) 业务模块禁止直连具体 LLM/ASR/TTS SDK，必须走 `shared_runtime` 接口；3) 业务模块的角色 ID / 语言 / 配置全部由调用方传入，不得硬编码"小暖"或任何单一 IP |
| **风险** | 初期开发速度变慢；examples 与契约测试会增加维护成本 |
...
| 规模档位 | 并发用户 | 日活 (DAU) | 消息吞吐 (msg/s) | 基础设施 |
|------|---------|-----------|-----------------|---------|
...
> ⚠️ **参考声明**：本章节仅用于估算不同能力模块在典型负载下的资源量级。API 单价、用户量级和毛利率都具有时效性，落地前必须按当时供应商价格、目标地区和产品策略重新核算。
...
| 定价策略 | 月费 | 日均成本 | 毛利率 | 适用场景 |
|---------|------|---------|--------|---------|
| 免费层 (限制20条/天) | ¥0 | ¥0.30 | -100% | 拉新获客 |
| 基础版 | ¥29.9/月 | ¥0.92 | 8% | 基础订阅验证 |
| 高级版 (语音+动作) | ¥69.9/月 | ¥1.50 | 36% | 主力营收 |
| 年费版 (全功能) | ¥599/年 | ¥1.50 | 62% | 稳定用户 |

> **参考结论**: 在上述假设下，纯文本模式成本较低，语音与动作能力会显著抬高边际成本。实际是否采用分层定价，应由目标用户、服务商价格和产品策略共同决定。
...
| 发布前评审 | 重要版本发布前 | 完整质量回归 + 红队测试 | 500+ | 全维度达标验证 |
